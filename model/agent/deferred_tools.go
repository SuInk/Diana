...
func (l *deferredToolLoader) Description() string {
	return "加载系统提示词「按需加载的工具」里列出的工具。names 传工具名；加载结果包含完整契约，随后只能通过 tools_execute(name,input) 调用；加载状态仅在当前运行有效。只加载这一轮确实要用的工具，不要为了看看有什么而加载。"
}
...
func (l *deferredToolLoader) Run(_ context.Context, input map[string]any) (string, error) {
	...
	result, err := json.Marshal(map[string]any{"loaded": contracts, "instruction": "使用 tools_execute，把目标工具名放入 name、参数对象放入 input；仅在当前 Run 有效。"})
	if err != nil {

...
func (cfg Config) WithDefaults() Config {
	...
	if strings.TrimSpace(cfg.MCPConfigPath) == "" {
		cfg.MCPConfigPath = filepath.Join(cfg.WorkDir, ".codex", "config.toml")
		if _, err := os.Stat(cfg.MCPConfigPath); err != nil {
			cfg.MCPConfigPath = filepath.Join(cfg.WorkDir, ".mcp.json")
		}
	}
	return cfg
}

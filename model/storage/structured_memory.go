...
func (s *SQLiteStore) EnqueueMemoryJob(ctx context.Context, payload assistant.MemoryJobPayload) (string, bool, error) {
	...
	if payload.Session == "" || (payload.Kind != assistant.MemoryJobEvent && payload.Kind != assistant.MemoryJobSummary) {
...
`, now.Unix(), active.ID); err != nil {
			...
			active.Status = assistant.MemoryStatusForgotten
...
`, now.Unix(), active.ID); err != nil {
		...
		item := assistant.StructuredMemoryItem{
			...
			Status:          assistant.MemoryStatusActive,
			CreatedAt:       now,
			UpdatedAt:       now,
		}
...
func normalizeMemoryCandidate(candidate assistant.MemoryCandidate, request assistant.MemoryWriteRequest) (assistant.MemoryCandidate, bool) {
	if candidate.Action == "" {
		candidate.Action = assistant.MemoryActionUpsert
	}
	if candidate.Action != assistant.MemoryActionUpsert && candidate.Action != assistant.MemoryActionForget {
	...
	if candidate.Key == "" || candidate.Topic == "" || (candidate.Action == assistant.MemoryActionUpsert && candidate.Content == "") {
		return assistant.MemoryCandidate{}, false
	}
	...
	case assistant.MemorySourceExplicit, assistant.MemorySourceInferred, assistant.MemorySourceSummary:
	default:
		candidate.SourceType = assistant.MemorySourceInferred
	}
	if candidate.Confidence < 0 || candidate.Confidence > 1 || candidate.Importance < 0 || candidate.Importance > 1 {
		return assistant.MemoryCandidate{}, false
	}
	if candidate.SourceType == assistant.MemorySourceInferred && candidate.Confidence < 0.9 {
		return assistant.MemoryCandidate{}, false
	}
	if candidate.SourceType != assistant.MemorySourceInferred && candidate.Confidence < 0.7 {
		return assistant.MemoryCandidate{}, false
	}
	if candidate.Visibility != assistant.MemoryVisibilityUser {
...
func insertMemoryItem(ctx context.Context, tx *sql.Tx, item assistant.StructuredMemoryItem) error {
...
func insertMemorySource(ctx context.Context, tx *sql.Tx, memoryID string, request assistant.MemoryWriteRequest, candidate assistant.MemoryCandidate, sourceTime time.Time, now time.Time) error {
...
func findMemoryBySourceAndKey(ctx context.Context, tx *sql.Tx, session string, messageID string, subjectUserID string, key string) (assistant.StructuredMemoryItem, bool, error) {
	var memoryID string
	err := tx.QueryRowContext(ctx, `
...
LIMIT 1`, session, messageID, strings.TrimSpace(subjectUserID), key).Scan(&memoryID)
	if errors.Is(err, sql.ErrNoRows) {
		return assistant.StructuredMemoryItem{}, false, nil
	}
	if err != nil {
		return assistant.StructuredMemoryItem{}, false, err
	}
	return findMemoryByID(ctx, tx, memoryID)
}

func findActiveMemory(ctx context.Context, tx *sql.Tx, scopeKey string, subjectUserID string, key string) (assistant.StructuredMemoryItem, bool, error) {
...
func findMemoryByID(ctx context.Context, tx *sql.Tx, id string) (assistant.StructuredMemoryItem, bool, error) {
...
func scanMemoryRow(scanner memoryScanner) (assistant.StructuredMemoryItem, bool, error) {
...
func scanStructuredMemory(scanner memoryScanner) (assistant.StructuredMemoryItem, error) {
	var item assistant.StructuredMemoryItem
	...
	if err != nil {
		return assistant.StructuredMemoryItem{}, err
	}
	...
	item.Kind = assistant.MemoryKind(kind)
	item.SourceType = assistant.MemorySourceType(sourceType)
	item.Visibility = assistant.MemoryVisibility(visibility)
	item.Status = assistant.MemoryStatus(status)
...
func memoryCandidateExpiry(candidate assistant.MemoryCandidate, sourceTime time.Time) time.Time {
	...
	if days == 0 {
		...
		case candidate.Kind == assistant.MemoryKindEpisode:
			days = 90
		case candidate.SourceType == assistant.MemorySourceInferred:
			days = 180
		}
	}
	if days <= 0 {
...
func memorySourceMessageID(request assistant.MemoryWriteRequest, candidate assistant.MemoryCandidate, sourceTime time.Time) string {
...
func memoryJobID(payload assistant.MemoryJobPayload) string {

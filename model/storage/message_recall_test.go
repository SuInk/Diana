...
func TestSQLiteStoreKeepsOriginalMessageAndRecallNotice(t *testing.T) {
	...
	original := assistant.MessageEvent{
		...
		MessageID:  "old-1",
		RawMessage: "持久化的撤回正文",
		Segments:   []assistant.MessageSegment{{Type: "text", Data: map[string]string{"text": "持久化的撤回正文"}}},
		SenderName: "Alice",
	}
	...
	recall := original
	recall.Kind = assistant.EventKindNotice
	...
	if len(all) != 1 || all[0].Kind != assistant.EventKindGroup {
	...
	if !ok || found.Kind != assistant.EventKindGroup || found.RawMessage != original.RawMessage {
...
func TestSQLiteStoreListsRecallsNewestFirst(t *testing.T) {
	...
	for _, event := range []assistant.MessageEvent{
		{Kind: assistant.EventKindNotice, SubType: "group_recall", Time: 100, GroupID: "123", MessageID: "older", RawMessage: "older"},
		{Kind: assistant.EventKindNotice, SubType: "group_recall", Time: 200, GroupID: "123", MessageID: "newer", RawMessage: "newer"},
	} {

...
func TestRuntimeSendsRecallSummaryWithFlatForgedForward(t *testing.T) {
	...
	messageIDs, err := runtime.sendNestedForwardPluginResponse(context.Background(), MessageEvent{Kind: EventKindGroup, GroupID: "123456", SelfID: "42"}, resp, "最近24小时共有两条撤回消息。", runtime.Config())
	if err != nil {
		t.Fatal(err)
	}
	if len(messageIDs) != 1 || messageIDs[0] != "42" {
...
func TestRuntimeFallsBackToTextRecallForwardWhenMediaForwardFails(t *testing.T) {
	...
	messageIDs, err := runtime.sendNestedForwardPluginResponse(context.Background(), MessageEvent{Kind: EventKindGroup, GroupID: "123456", SelfID: "42"}, resp, "最近有一条图片撤回。", runtime.Config())
	if err != nil {
		t.Fatal(err)
	}
	if len(messageIDs) != 1 || messageIDs[0] != "902" {
		t.Fatalf("message ids = %#v", messageIDs)
	}

...
func sanitizeDebugGenerateRequest(req llm.GenerateRequest) llm.GenerateRequest {
	...
	for index, message := range req.Messages {
		...
		cloned.Messages[index].ToolCalls = cloneDebugToolCalls(message.ToolCalls)
...
func sanitizeDebugGenerateResponse(req llm.GenerateRequest, response *llm.GenerateResponse) *llm.GenerateResponse {
	...
	cloned := *response
	cloned.ToolCalls = cloneDebugToolCalls(response.ToolCalls)
...
// cloneDebugToolCalls 复制工具调用，避免调试记录和后续请求共用同一份参数。
func cloneDebugToolCalls(calls []llm.ToolCall) []llm.ToolCall {

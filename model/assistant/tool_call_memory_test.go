...
func TestToolCallMemoryTellsNextTurnWhatWasSearched(t *testing.T) {
	...
	for _, want := range []string{"20:06 web_search「iCloud 由云上贵州运营 条款与条件 更新」", "browser_render「https://www.apple.com/legal/」（调用失败）", "不要因为对方质疑就改口认错"} {
		if !strings.Contains(context, want) {
			t.Fatalf("context missing %q:\n%s", want, context)
		}
	}
	for _, unwanted := range []string{"chat_history", "agent.finalize", "回复正文"} {
		if strings.Contains(context, unwanted) {
			t.Fatalf("context should not include %q:\n%s", unwanted, context)
		}
	}
	// 最近的调用排在前面。
	if strings.Index(context, "browser_render") > strings.Index(context, "web_search") {
...
func TestToolCallMemoryExpiresAndCaps(t *testing.T) {
	...
	for index := 0; index < recentToolCallLimit+3; index++ {
		runtime.rememberToolCalls(event, []agent.Step{{Tool: "web_search", Input: map[string]any{"query": strings.Repeat("长", 120)}}})
	}
	context := runtime.toolCallContext(event)

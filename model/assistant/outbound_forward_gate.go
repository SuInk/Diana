...
// 根本不存在，于是站外内容一路直达群里。
//
// 这里不跟机器人级的审核总开关走。总开关管的是回复链路；转发卡片走的是
...
func (r *Runtime) auditForwardNodesSafety(ctx context.Context, event MessageEvent, nodes []map[string]any) error {
	...
	// 转发卡片只审账号安全，不判空转也不判收尾：它不是一来一回的对话。
	decision, err := r.runReplyAudit(ctx, event, readableEventText(event, event.RawMessage), text, cfg, botReplyLoopEvidence{}, replyAuditNeed{AccountSafety: true})
	if err != nil {

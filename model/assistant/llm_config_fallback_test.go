...
// 供应商没变就只换主模型。后备路由是在 WebUI 里一条条排出来的，不能被一句
// 「换个模型」顺手清掉，顺序也不能变。点名当前这家供应商也是同一回事——主人
// 要的是换模型，没有一句在说把后备清掉。
func TestChatModelSwitchKeepsFallbackRoutes(t *testing.T) {
	for name, input := range map[string]map[string]any{
		"没点名供应商": {"model": "other"},
		"点名当前这家": {"provider_id": "one", "model": "other"},
	} {
		t.Run(name, func(t *testing.T) {
			...
			reply, err := newDianaLLMConfigTool(r, event).Run(context.Background(), input)
			if err != nil {
				t.Fatal(err)
			}
...
// 换到另一家供应商时，原有的后备路由确实会作废——那是设计好的，但必须在回执里
// 说出来，不能等主模型挂了才发现没有兜底。

...
func TestGroupConfigOverridesRecallReplyAutoDeletePolicy(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "inherits global defaults", baseEnabled: &enabled, baseDelay: 60, wantEnabled: true, wantDelay: 60},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if recallReplyShouldAutoDelete(effective, responses) != tt.wantEnabled {
				t.Fatalf("auto-delete decision did not match effective config")
			}
...
func TestLegacyGroupConfigInheritsRecallReplyAutoDeletePolicy(t *testing.T) {
	disabled := false
	base := BotConfig{
		RecallReplyAutoDeleteEnabled: &disabled,
		RecallReplyTTLSeconds:        180,
	}
	store := &testWritableGroupConfigStore{set: GroupConfigSet{Groups: []GroupConfig{{
		GroupID:    "123",
		Enabled:    true,
		EnabledSet: true,
	}}}}
	...
	effective := runtime.effectiveConfigForEvent(MessageEvent{Kind: EventKindGroup, GroupID: "123"})
	if effective.RecallReplyAutoDeleteEnabled == nil || *effective.RecallReplyAutoDeleteEnabled {
		t.Fatalf("legacy group enabled = %#v", effective.RecallReplyAutoDeleteEnabled)
	}
	if effective.RecallReplyTTLSeconds != 180 {
		t.Fatalf("legacy group delay = %d", effective.RecallReplyTTLSeconds)
	}
}

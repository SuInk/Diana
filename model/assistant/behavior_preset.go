...
//
// 措辞要和 splitChatReply 认的边界一字不差地对上：短回复里的换行会分条，清单、
// 步骤、代码整块发。两边理解不一样的话，分条位置就又变回看模型的排版习惯了——
// 「空行分条」当年就是这么被删掉的。
const replySegmentationRule = "回复包含多个意群时（例如先给结论、再讲理由，或者先解释再补一句常见用法），把它们分开发出去，像真人连发几条那样，不要挤进同一条消息：在意群边界换行就会分成两三条，也可以显式写 " + notificationSplitMarker + "。反过来，编号或项目符号列表、一组步骤、代码和报错原文是一个整体，照常用换行排版即可，它们会整块发出去，不要在每个列表项前写 " + notificationSplitMarker + "。"
...
func (style ReplyStyle) stylePrompt() string {
	...
	case ReplyStyleGroupmate:
		return strings.Join([]string{
			...
			"连着说两三句短的时候，那就是两三次独立发言，句与句之间换行或写 <dianabr>，会分成几条消息发出去，像真人连发那样。同一条消息里需要换行排版的只有清单、步骤和代码，它们会整块发出去。",

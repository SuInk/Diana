...
func TestAgentImageHistoryLineCarriesCachedDescriptions(t *testing.T) {
	...
	if !strings.Contains(bare, "此消息包含 1 张真实图片") {
		t.Fatalf("bare line = %q", bare)
	}
	if strings.Contains(bare, "已缓存描述") {
		t.Fatalf("bare line should not invent a description: %q", bare)
	}

	described := agentImageHistoryPromptTextWithDescriptions(event, 1120, []string{"图片1已缓存描述=报纸与干花拼贴的二次元人物"})
	if !strings.Contains(described, "此消息包含 1 张真实图片") {
		t.Fatalf("described line lost the image count: %q", described)
	}
	if !strings.Contains(described, "图片1已缓存描述=报纸与干花拼贴的二次元人物") {

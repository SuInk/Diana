...
var promptStickerInstructionSpec = registerPrompt(PromptSpec{
	...
	Group:   PromptGroupMedia,
	Title:   "表情包标注 · 要求",
	Usage:   "和表情包图片一起发给视觉模型的简介要求。机器人挑表情时按这份简介和末尾「标签：」那一行的关键词检索；改写时保留标签行，否则只能按简介原文匹配。",
	Default: "请为这张聊天表情包生成简短中文简介。重点说明发送者借这张图表达的潜台词、复合情绪、说话视角、典型触发场景和清晰可辨的原始文字，而不是只描述构图或画风。不要回答当前聊天问题，不要使用 Markdown，简介控制在180字以内。简介之后另起一行，以「标签：」开头，用顿号分隔写 4 到 8 个检索用短词，覆盖情绪、动作、角色和适用场景，图中有字就把原文也列进去。",
})

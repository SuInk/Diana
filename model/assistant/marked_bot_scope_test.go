...
// markedBotRuntime 里 30004 标在机器人级：那是「这个账号在哪儿都是机器人」的
...
func markedBotRuntime(t *testing.T, provider LLMProvider) *Runtime {
	...
	if _, err := store.SaveGroupConfig(GroupConfig{
		BotProfileID: "a", GroupID: "20003", Enabled: true, EnabledSet: true,
...
func TestMarkerScopeFollowsWhereItWasSet(t *testing.T) {
	...
	private := MessageEvent{Kind: EventKindPrivate, ProfileID: "a", UserID: "30004", MessageID: "p1"}
	if !runtime.accountMarkedAsBot(private) {
		t.Fatal("机器人级标记应当在私聊里也生效")
	}
	// 只标在某个群里的账号：那个群里算机器人，别的群和私聊都不算。
	inGroup := MessageEvent{Kind: EventKindGroup, ProfileID: "a", GroupID: "20003", UserID: "700000001", MessageID: "g1"}
	if !runtime.accountMarkedAsBot(inGroup) {
		t.Fatal("群级标记应当在本群生效")
	}
	otherGroup := inGroup
	otherGroup.GroupID = "20004"
...
func TestMarkedBotPrivateMessageSuppressedUnlessAddressed(t *testing.T) {
	...
	event := MessageEvent{
		Kind: EventKindPrivate, ProfileID: "a", UserID: "30004", MessageID: "p1",
...
func TestBotReplyLoopCandidateCoversPrivate(t *testing.T) {
	...
	rememberPrivateBotReply(runtime, "30004", "bot-loop", time.Now())
...
func TestReplyAuditPassesMarkerEvidenceForLoop(t *testing.T) {
	...
	event := MessageEvent{
		Kind: EventKindPrivate, ProfileID: "a", UserID: "30004", MessageID: "p2", ToMe: true,
		Time: time.Now().Unix(), RawMessage: "嗯 拜",
		Segments: []MessageSegment{{Type: "text", Data: map[string]string{"text": "嗯 拜"}}},
	}
	rememberPrivateBotReply(runtime, "30004", "bot-marker", time.Now())

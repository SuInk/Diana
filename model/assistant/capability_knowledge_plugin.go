...
var coreCapabilityDocuments = []capabilityDocument{
	...
	{ID: "core:thread-state", Title: "私有多轮任务状态", Content: "通过 diana.thread_state 保存 Diana 自己为当前用户和会话创建的短期私有状态，例如猜谜目标、临时计划和表单进度。状态会跨消息和进程重启恢复，完成、取消或超时后清理，不写入用户长期记忆，也不会出现在公开回复里；管理员可在事件详情审计本轮实际调用的状态。", Source: "core", Enabled: true},

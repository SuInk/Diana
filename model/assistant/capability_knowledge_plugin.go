...
var coreCapabilityDocuments = []capabilityDocument{
	...
	{ID: "core:group", Title: "群资料与成员", Content: "OneBot 群资料和成员按 onebot-v11 skill 使用 diana.onebot_v11 的 get_group_info、get_group_member_list、get_group_member_info；其他平台使用只读 diana.group。回复欲望、评分门槛和冷却统一由 diana.bot_config 更新 participation；关闭主动插话用 desire_level=off，不用平台禁言。TG 的 members 是管理员与已知账号候选，不是完整名单；实时成员验证使用 member。头像来源通过图片工具指定，由运行时按平台获取；可对可用候选进行本地头像图片匹配，不能把部分候选当成全群。", Source: "core", Enabled: true},

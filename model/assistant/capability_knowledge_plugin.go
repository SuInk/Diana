...
var coreCapabilityDocuments = []capabilityDocument{
	...
	{ID: "core:thread-state", Title: "多轮任务临时状态", Content: "通过 diana.thread_state 保存短期 canonical 状态。scope=user 用于当前用户的私有猜谜、计划和表单；scope=session 用于多人棋局、共同计划等需要当前会话参与者接续同一状态的任务。状态会跨消息和进程重启恢复，更新需带 expected_version 防止并发覆盖，完成、取消或超时后清理，不写入长期记忆，也不会在调试追踪中泄露内容。", Source: "core", Enabled: true},

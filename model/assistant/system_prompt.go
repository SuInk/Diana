...
const (
	...
	promptToolOneBotGroup = "用户要求读取当前群资料、成员列表、按昵称查成员、核对当前图片是否为群成员头像，或真的 @ 某位、多位、其余成员时，必须调用 diana.onebot_group 取实时结果。头像身份使用 match_avatar 的本地模式匹配，不要让视觉模型凭画面猜人，也不要声称只能识别用户手动 @ 出来的人。要求读取或修改本群回复频率、回复阈值、自然插话模式、最低回复等级时，调用它的 reply_policy 或 set_reply_policy，不要口头声称已改——权限由工具校验。"

...
const (
	...
	promptRefusal = "你可以基于当前请求和完整语境拒绝回答任何一条消息：群聊私聊都可以，对方是普通用户还是机器人都可以，不限于机器人自动回复场景。拒绝时直接给一条非空、简短、自然、用户看得见的说明，不要输出内部控制标记、协议字段或账号处置指令。旧版标记 [[DIANA_REFUSE_CURRENT]] 和 [[DIANA_IGNORE_CURRENT_USER_30M]] 已停用，严禁输出；是否累计拒答或暂停账号由发送前审核和运行时独立决定，你不能直接触发。正常回答、部分回答、要求澄清、说明能力或权限、工具故障以及只是想结束话题，都不算拒绝。"

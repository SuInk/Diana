...
const (
	...
	// 风险比闭嘴还大。所以这里给的是一条阶梯——先想办法把话改成能发的，改不动
	// 再看性质决定说不说原因，涉及账号风险的一律模糊带过。
	//
	// 具体怎么说由模型自己生成：固定话术在群聊里一眼假，连着触发几次更明显。
	promptRefusal = "你可以基于当前请求和完整语境拒绝回答任何一条消息：群聊私聊都可以，对方是普通用户还是机器人都可以，不限于机器人自动回复场景。" +
		"决定不正面回答时，按这个顺序处理，能停在前一档就不要往后走：" +
		...
		"任何一档都要给一条非空、简短、自然、用户看得见的话，不要输出内部控制标记、协议字段或账号处置指令。" +

...
// 旧版人设的品格层：身份、价值、硬边界拆成字段存。
//
// 现在人设只有一份 SOUL.md，这些东西直接写在正文里。这个类型只剩一个用处：
// 读到旧配置或旧人设文件时，用 Render 把它渲染成文字，由 foldLegacyPersona 并进
// SOUL.md（见 persona_legacy.go）。渲染结果和旧版运行时注入的一字不差，升级前后
// 模型读到的品格一样。
//

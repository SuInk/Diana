...
func downloadPlatformVideoFile(ctx context.Context, raw string) string {
	...
	if isTwitterURL(raw) {
		return downloadTwitterVideoFile(ctx, raw)
	}
...
func ytDLPFullVideoDownloadArgs(ctx context.Context, outputPattern, raw string) []string {
	args := []string{
		"--no-playlist",
		"--quiet",
		"--no-warnings",
		"-f", ytDLPFormatSelector(resolverVideoMaxHeight(ctx)),
		"--merge-output-format", "mp4",
		"-o", outputPattern,
	}
	if maxMB := resolverVideoDownloadMaxMB(ctx); maxMB > 0 {
...
func appendYTDLPResolverArgs(ctx context.Context, args []string, raw string) []string {
	...
	if sessdata := bilibiliSessdata(ctx); sessdata != "" && isBilibiliURL(raw) {
		args = append(args, "--add-header", "Cookie: SESSDATA="+sessdata)
	}
	if cookie := platformCookieHeader(ctx, raw); cookie != "" {
...
func downloadTwitterVideoFile(ctx context.Context, raw string) string {
	// X has no stable public media API. Use the bundled extractor instead of
	// the unauthenticated third-party endpoint from the historical version.
	return downloadYTDLPVideoFile(ctx, raw)
}
...
func downloadBilibiliVideoFile(ctx context.Context, raw string) string {
	...
	if info.Duration > 0 && info.Duration > float64(resolverVideoMaxDuration(ctx)) {
		log.Printf("resolver bilibili video too long: %.1fs > %ds", info.Duration, resolverVideoMaxDuration(ctx))
		return ""
	}
	video, audio := selectBilibiliFormats(resolverVideoMaxHeight(ctx), info)
	if video.URL == "" {
		return ""
	}
	...
	headers := bilibiliDownloadHeaders(ctx, info.HTTPHeaders)
	if !downloadURLToFile(ctx, video.URL, videoPath, mergeHeaders(headers, video.HTTPHeaders), resolverVideoDownloadMaxBytes(ctx)) {
		_ = os.RemoveAll(workDir)
		return ""
	}
	if audio.URL != "" {
		if !downloadURLToFile(ctx, audio.URL, audioPath, mergeHeaders(headers, audio.HTTPHeaders), resolverVideoDownloadMaxBytes(ctx)) {
			_ = os.RemoveAll(workDir)
			return ""
		}
	...
	if !videoFileAllowed(ctx, outputPath) {
		_ = os.RemoveAll(workDir)
		return ""
	}
	return outputPath
...
func downloadBilibiliVideoFileViaAPI(ctx context.Context, raw string) string {
	...
	if duration > 0 && duration > resolverVideoMaxDuration(ctx) {
		log.Printf("resolver bilibili video too long: %ds > %ds", duration, resolverVideoMaxDuration(ctx))
		return ""
	}
	...
	outputPath := filepath.Join(workDir, "video.mp4")
	headers := bilibiliDownloadHeaders(ctx, nil)
	video, audio := selectBilibiliDashMedia(resolverVideoMaxHeight(ctx), play.Data.Dash.Video, play.Data.Dash.Audio)
	if video.Base() != "" {
		videoPath := filepath.Join(workDir, "video.m4s")
		audioPath := filepath.Join(workDir, "audio.m4s")
		if !downloadBilibiliCandidateURLs(ctx, video.URLs(), videoPath, headers, resolverVideoDownloadMaxBytes(ctx)) {
			_ = os.RemoveAll(workDir)
			return ""
		}
		if audio.Base() != "" {
			if !downloadBilibiliCandidateURLs(ctx, audio.URLs(), audioPath, headers, resolverVideoDownloadMaxBytes(ctx)) {
				_ = os.RemoveAll(workDir)
				return ""
			}
			if !mergeMediaToMP4(ctx, videoPath, audioPath, outputPath) {
		...
		if !downloadBilibiliCandidateURLs(ctx, append([]string{play.Data.DURL[0].URL}, play.Data.DURL[0].BackupURL...), outputPath, headers, resolverVideoDownloadMaxBytes(ctx)) {
			_ = os.RemoveAll(workDir)
			return ""
		}
	} else {
...
func fetchBilibiliJSON(ctx context.Context, raw string, referer string, out any) bool {
	...
	for key, value := range bilibiliDownloadHeaders(ctx, nil) {
...
func selectBilibiliDashMedia(maxHeight int, videos []bilibiliDashMedia, audios []bilibiliDashMedia) (bilibiliDashMedia, bilibiliDashMedia) {
	var video bilibiliDashMedia
	for _, candidate := range videos {
		if candidate.Base() == "" || (maxHeight > 0 && candidate.Height > maxHeight) {
			continue
		}
...
func selectBilibiliFormats(maxHeight int, info ytdlpInfo) (ytdlpFormat, ytdlpFormat) {
	...
	for _, format := range info.Formats {
		...
		if format.VCodec != "" && format.VCodec != "none" && (format.ACodec == "" || format.ACodec == "none") {
			if format.Height <= 0 || (maxHeight > 0 && format.Height > maxHeight) {
				continue
			}
...
func bilibiliDownloadHeaders(ctx context.Context, base map[string]string) map[string]string {
	headers := map[string]string{
		"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36",
		"Referer":    "https://www.bilibili.com",
	}
	if sessdata := bilibiliSessdata(ctx); sessdata != "" {
...
func platformCookieHeader(ctx context.Context, raw string) string {
	parsed, err := url.Parse(raw)
	if err != nil {
		return ""
	}
...
func downloadGenericVideoFile(ctx context.Context, raw string, headers map[string]string) string {
	...
	outputPath := filepath.Join(workDir, "video.mp4")
	if !downloadURLToFile(ctx, raw, outputPath, headers, resolverVideoDownloadMaxBytes(ctx)) || !videoFileAllowed(ctx, outputPath) {
		_ = os.RemoveAll(workDir)
		return ""
	}
	return outputPath
...
func videoFileAllowed(ctx context.Context, path string) bool {
	info, err := os.Stat(path)
	if err != nil || info.IsDir() || info.Size() == 0 || info.Size() > resolverVideoMaxBytes(ctx) {
		return false
	}
	return true
}

func resolverVideoMaxBytes(ctx context.Context) int64 {
	return int64(resolverVideoMaxMB(ctx)) * 1024 * 1024
}

func resolverVideoMaxMB(ctx context.Context) int {
	if limits, ok := ctx.Value(resolverMediaLimitsKey{}).(resolverMediaLimits); ok && limits.MaxMB > 0 {
		return limits.MaxMB
	}
...
func resolverVideoDownloadMaxBytes(ctx context.Context) int64 {
	maxMB := resolverVideoDownloadMaxMB(ctx)
	if maxMB <= 0 {
		return 0
	}
...
func resolverVideoMaxDuration(ctx context.Context) int {
	if limits, ok := ctx.Value(resolverMediaLimitsKey{}).(resolverMediaLimits); ok && limits.MaxDurationSeconds > 0 {
		return limits.MaxDurationSeconds
	}
	return envInt("DIANA_RESOLVER_VIDEO_MAX_DURATION", defaultVideoMaxDuration)
}

type resolverMediaLimits struct {
	MaxMB              int
	MaxDurationSeconds int
	// MaxHeight 为清晰度上限，0 表示不限。
	MaxHeight int
}

type resolverMediaLimitsKey struct{}

func withResolverMediaLimits(ctx context.Context, maxMB, maxDurationSeconds, maxHeight int) context.Context {
	return context.WithValue(ctx, resolverMediaLimitsKey{}, resolverMediaLimits{
		MaxMB:              maxMB,
...
// resolverVideoMaxHeight 返回清晰度上限，0 表示不限。
func resolverVideoMaxHeight(ctx context.Context) int {
	if limits, ok := ctx.Value(resolverMediaLimitsKey{}).(resolverMediaLimits); ok && limits.MaxHeight >= 0 {
		return limits.MaxHeight
	}
	return defaultVideoMaxHeight
}

// ytDLPFormatSelector 按清晰度上限拼 yt-dlp 的 -f 表达式，0 表示不限。
func ytDLPFormatSelector(maxHeight int) string {
	if maxHeight <= 0 {
		return "bv*+ba/b/best"
	}
	return fmt.Sprintf("bv*[height<=%d]+ba/b[height<=%d]/best[height<=%d]/best", maxHeight, maxHeight, maxHeight)
}
...
func credentialFromContext(ctx context.Context, pick func(resolverCredentials) string) string {
	...
	creds, ok := ctx.Value(resolverCredentialsKey{}).(resolverCredentials)
	if !ok {
		return ""
	}
	return strings.TrimSpace(pick(creds))
}
...
func isBilibiliURL(raw string) bool {
	return urlMatchesDomain(raw, "bilibili.com", "b23.tv", "bili2233.cn")
}

func isDouyinURL(raw string) bool {
	return urlMatchesDomain(raw, "douyin.com")
}
...
func isTwitterURL(raw string) bool {
	return urlMatchesDomain(raw, "x.com", "twitter.com")
}

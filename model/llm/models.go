...
func requestOpenAICompatibleModels(ctx context.Context, httpClient *http.Client, baseURL, endpoint, apiKey, userAgent string, headers map[string]string) ([]ModelInfo, error) {
	...
	if err != nil {
		return nil, modelListDecodeError{
			requestURL:  req.URL.String(),
			statusCode:  resp.StatusCode,
			contentType: resp.Header.Get("Content-Type"),
			body:        string(body),
...
func (e modelListHTTPError) Error() string {
	return fmt.Sprintf(
		...
		firstNonEmptyModelDetail(e.contentType, "未知"),
		responsePreview(e.body),
	)
}

type modelListDecodeError struct {
	requestURL  string
	statusCode  int
	contentType string
	body        string
...
func (e modelListDecodeError) Error() string {
	return fmt.Sprintf(
		"模型列表响应无法解析\n请求：GET %s\n状态：%d %s\n响应类型：%s\n解析错误：%v\n响应：%s",
		e.requestURL,
		e.statusCode,
...
func isModelListFallbackError(err error) bool {
	if _, ok := err.(modelListDecodeError); ok {
		return true
	}
...
func responsePreview(body string) string {
	const limit = 600
...
func firstNonEmptyModelDetail(values ...string) string {

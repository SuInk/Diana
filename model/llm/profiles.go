...
// 语义分组：default 即对话组；其余按用途路由，组内顺序就是降级优先级。
const (
...
func (s ProfileSet) GroupProfiles(group string) []Profile {
	group = NormalizeProfileGroup(group)
	out := make([]Profile, 0, len(s.Profiles))
	for _, profile := range s.Profiles {
		if NormalizeProfileGroup(profile.Group) != group {
			continue
...
// Reorder 按给定 ID 顺序重排配置集；未出现在列表里的配置保持原相对顺序放在末尾。
func (s ProfileSet) Reorder(ids []string) ProfileSet {
	if len(ids) == 0 {
		return s
	}
	index := make(map[string]int, len(ids))
	for i, id := range ids {
		index[strings.TrimSpace(id)] = i
	}
	ordered := make([]Profile, 0, len(s.Profiles))
	rest := make([]Profile, 0, len(s.Profiles))
	for _, profile := range s.Profiles {
		if _, ok := index[profile.ID]; ok {
			ordered = append(ordered, profile)
		} else {
			rest = append(rest, profile)
		}
	}
	slicesSortStableByIndex(ordered, index)
	s.Profiles = append(ordered, rest...)
	return s
}

// slicesSortStableByIndex 按 index 给定的顺序稳定排序。
func slicesSortStableByIndex(profiles []Profile, index map[string]int) {
	for i := 1; i < len(profiles); i++ {
		for j := i; j > 0 && index[profiles[j].ID] < index[profiles[j-1].ID]; j-- {

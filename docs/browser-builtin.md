...
| | 一次性无头渲染（`browser_render`） | 浏览器控制扩展（`browser_ext_*`） | 内置浏览器（本页） |
| --- | --- | --- | --- |
...
什么都不用配。扩展那一档另外要在每台机器人上打开 `agent_browser_control_enabled`（默认关）；
轮到扩展、而这台机器人没打开时，它这一轮没有交互式浏览器，不会退回内置浏览器。选中的那个用不了（内置浏览器没勾或找不到 Chrome；扩展没连上或正被接管）而另一个开着时，这一轮
自动换成另一个（`model/browsersource.Pick`，顺序存在 `app_state` 的 `browser_source` 里）。
内置浏览器被接管不算用不了：这时不换来源，`browser_*` 调用直接报接管中。
...
就允许，想让某台机器人彻底不碰它，把它的 `agent_browser_box_disabled` 勾上——勾上后这台机器人的
`browser_open` 这组工具改接它配置里的外部 CDP 地址，那里没有浏览器时调用会失败。
...
- 画面和输入照常，因为那正是你要用的；
- 交还之后机器人才能继续。忘了交还也没关系：5 分钟没有任何输入，接管自动交还给机器人
  （记一条日志）；要继续用，再点一下画面就重新接管。

没接管时，画面上的鼠标悬停和滚轮不会转发给浏览器，免得你只是看一眼就打断机器人。
...
- **你的动作**：打开浏览器页时的自动拉起、接管和交还（`browser_box_start` / `_takeover`），在
  ...
  鼠标移动不逐条记。
- 一次性无头渲染（`browser_render`）不带登录态，不混进这里：网页渲染插件被动渲染消息里的
  链接时由插件自己记；Agent 主动调用它时只留通用的 `agent_tool` 记录。

...
1. 在 WebUI 的「浏览器」页把来源选成「我自己的 Chrome」，在下面创建一把令牌，记下明文（只显示这一次）。
...
**没有截图。** 浏览器的截图接口（`captureVisibleTab`）要么要 `<all_urls>`、要么要 `activeTab`，都是「当前这一页随便读」级别的权限，比「只授权白名单站点」宽得多；为一张图把扩展权限放大到全网不值得。要页面内容用 `browser_ext_read`；要出图用 `browser_screenshot`，它跑在 Diana 自己的一次性浏览器里，不碰你的登录态。

...
0. 容器部署没有仓库检出：在 Diana 的「浏览器 → 浏览器控制扩展」点「下载扩展源码包」，解压后就是本目录的内容。
...
- 不截图：浏览器的截图接口要 `<all_urls>` 或 `activeTab` 级权限，与「只授权白名单站点」冲突。
- 只上报白名单内标签页的地址和标题，其余标签页不出现在任何帧里。
- 接管打开时一条指令都不执行。
- 站点权限只申请白名单里的 origin；白名单变了要重新申请。申请的模式必须落在 manifest 的 `optional_host_permissions` 里，所以每个站点拆成 http、https 两条，不能用 scheme 通配——通配模式 Chrome 会直接拒收。

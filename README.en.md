...
**Administrator quick login:** enable the master switch on the Assistant page and configure the owner account; the two methods then toggle independently. Private-message confirmation is on by default: the browser shows a six-digit code, the owner sends it privately to the bot, and sending it logs the browser in, and the bot replies with a receipt naming the source IP and device. The receipt needs no action; it exists so the message is not swallowed silently — if the code was relayed under false pretenses, the owner sees it immediately and can revoke the session and change the password. Server-pushed code delivery is off by default: it makes the server message the owner, and the endpoint needs no credentials, so on a public deployment anonymous requests can use it to spam the owner or hold the 60-second cooldown window and lock the owner out. Codes for both methods expire after five minutes, are single-use, and require the active bot to be online.

...
**Administrator quick login:** enable the master switch on the Assistant page and configure the owner account; the two methods then toggle independently. Private-message confirmation is on by default: the browser shows a six-digit code, the owner sends it privately to the bot, and the bot reports the request's source IP and device before asking the owner to reply `确认 <code>` to approve — the code is required so an offhand `确认` in ordinary conversation cannot approve a login or `取消` to reject (the code stays on the web page — the bot never echoes it back). That extra step exists so the owner can see who is logging in instead of blind-signing a bare number. Server-pushed code delivery is off by default: it makes the server message the owner, and the endpoint needs no credentials, so on a public deployment anonymous requests can use it to spam the owner or hold the 60-second cooldown window and lock the owner out. Codes for both methods expire after five minutes, are single-use, and require the active bot to be online.

...
- **Music**: two features sharing one "turn a song into a voice message" path. **Link resolution** — a NetEase song link shared in chat is downloaded and sent as a voice message, so nobody has to leave the conversation to hear it; it recognizes the web, single-page-app anchor, mobile path and `163cn.tv` short-link forms, and will not mistake an artist, album or playlist page for a track. **Song requests** — "put on Dao Xiang", "play me something for winding down" share no common keyword, so instead of a synonym list the model decides for itself through the `diana.music` tool whether to play something and what; ordinary members can use it too, and it can be switched off in the plugin settings. Without configuration it uses the official endpoints, which only serve previewable tracks; point it at a self-hosted NeteaseCloudMusicApi with a `MUSIC_U` cookie for member-quality audio. Duration, file size, bitrate and whether to announce the song title are all configurable; set a Silk encoder path to transcode to Tencent Silk first, or leave it empty and let the OneBot client convert. Voice only plays correctly on OneBot v11.

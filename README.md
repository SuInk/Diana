...
外部系统（CI、监控、脚本）可以通过 HTTP 接口让机器人把消息推送到指定会话，把 Diana 当作通知出口。密钥在「设置 → 安全 → 对外 API」里创建，明文只在创建时显示一次，存储里只保留 SHA-256 哈希；吊销立即生效，调用记录进日志中心（`actor` 为 `openapi:<密钥名>`）。
...
- 单把密钥限流每分钟 60 次；超限返回 `429` 并带 `Retry-After`。接口只做投递，不触发模型调用。

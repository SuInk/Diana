...
**管理员快速登录**：在「机器人」页开启该功能并配置主人账号后，登录页提供「私聊确认」方式：网页显示 6 位验证码，主人将其私聊发送给机器人后自动登录。验证码 5 分钟有效、一次性使用，创建操作按 IP 限流；需要当前机器人在线。此前由服务端主动下发验证码的登录方式已移除——该端点无需任何凭证即可触发，会被匿名请求用来骚扰主人账号或抢占冷却窗口拒绝主人本人登录。豁免路径：登录及管理员快速登录端点、`/api/health`（监控探活）、`/onebot/*`（由 OneBot access token 单独鉴权）、群管理页（自有群验证码流程）。会话 cookie 未设 Secure 以兼容内网 HTTP，公网部署请套 HTTPS 反向代理。

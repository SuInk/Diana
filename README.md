...
**slim 轻量镜像：** 同一仓库同时发布 `-slim` 变体（如 `ghcr.io/suink/diana:latest-slim`、`ghcr.io/suink/diana:v0.8.126-slim`）：不预装 Chromium、Noto CJK 字体、ffmpeg、yt-dlp 与 tesseract，体积约为完整版的五分之一（拉取 67 MB / 落盘 274 MB，完整版 447 MB / 1.48 GB），适合不需要网页渲染、媒体下载和 OCR 的部署。上面那条 `DIANA_VARIANT=slim` 的安装命令就是选它，变体会写进部署目录的 `.env`（`DIANA_IMAGE=`），以后 `docker compose pull` 自动跟着走；想换回完整版把那一行删掉、或重跑安装命令时带 `DIANA_VARIANT=full`。内置浏览器这一档在 slim 上会明确报「找不到浏览器」并给出安装命令，不会悄悄失效。之后想用网页渲染，在宿主机执行 `docker exec -u root <容器名> apk add --no-cache chromium font-noto-cjk` 即可（WebUI 依赖管理里点一键安装会因进程非 root 失败，报错会直接附上这条命令）。注意容器重建后需重新安装，数据在挂出的 `data/` 里不受影响。

...
**管理员快速登录**：在「机器人」页开启该功能并配置主人账号后，登录页提供两种方式。默认的「验证码登录」会把 6 位一次性验证码发送到主人 QQ 或 Telegram，浏览器还必须持有本次请求的 challenge token 才能兑换会话；「私聊确认」则由网页显示验证码，主人将其私聊发送给机器人后自动登录。验证码 5 分钟有效、限制尝试次数且成功后立即作废，发送操作有 60 秒冷却；两种方式都需要当前机器人在线。豁免路径：登录及管理员快速登录端点、`/api/health`（监控探活）、`/onebot/*`（由 OneBot access token 单独鉴权）、群管理页（自有群验证码流程）。会话 cookie 未设 Secure 以兼容内网 HTTP，公网部署请套 HTTPS 反向代理。

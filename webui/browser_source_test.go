...
func TestBrowserSourceSwitchesAreIndependent(t *testing.T) {
	...
	// 扩展排在前面但没连上，内置浏览器也没开：这一轮不用浏览器。
	if state.Active != browsersource.Off {
		t.Fatalf("扩展没连上时应按顺序落到下一个：%+v", state)
	}
	state = putBrowserSource(t, router, `{"extension_enabled":false}`)
	if state.Extension.Enabled || state.Active != browsersource.Off {

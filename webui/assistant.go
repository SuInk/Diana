...
func (h *BotHandler) activateProfile(c *gin.Context) {
	var payload assistant.ConfigPayload
	if err := c.ShouldBindJSON(&payload); err != nil {
		h.writeError(c, http.StatusBadRequest, "profile_activate", err, "", nil)
		return
	}
	targetID := strings.TrimSpace(payload.ID)
	if targetID == "" {
		h.writeError(c, http.StatusBadRequest, "profile_activate", fmt.Errorf("profile id is required"), "", nil)
		return
	}
	next := h.profiles.Profiles().WithActive(targetID)
	current, ok := next.Current()
	if !ok || current.ID != targetID {
		h.writeError(c, http.StatusNotFound, "profile_activate", fmt.Errorf("profile %q not found", targetID), targetID, nil)
		return
	}
	if err := h.applyProfileSet(next); err != nil && !errors.Is(err, assistant.ErrBotDisabled) {
		h.writeError(c, http.StatusBadRequest, "profile_activate", err, botLogTarget(current), botLogMetadata(current))
		return
	}
	if err := h.profiles.SaveProfiles(next); err != nil {
		h.writeError(c, http.StatusInternalServerError, "profile_activate", err, botLogTarget(current), botLogMetadata(current))
		return
	}
	recordRequestOperation(c, h.logs, "profile_activate", "OneBot v11 机器人配置已切换", targetID, botLogMetadata(current))
	c.JSON(http.StatusOK, assistant.PayloadFromProfileSet(next))
}
...
func (h *BotHandler) deleteProfile(c *gin.Context) {
	...
	current, ok := next.Current()
	if !ok {
		h.writeError(c, http.StatusBadRequest, "profile_delete", fmt.Errorf("diana profile set is empty"), "", nil)
		return
	}
	if err := h.applyProfileSet(next); err != nil && !errors.Is(err, assistant.ErrBotDisabled) {
		h.writeError(c, http.StatusBadRequest, "profile_delete", err, botLogTarget(current), botLogMetadata(current))
		return
	}
	if err := h.profiles.SaveProfiles(next); err != nil {
...
func (h *BotHandler) start(c *gin.Context) {
	if err := h.runtime.Start(h.ctx); err != nil {
		h.writeError(c, http.StatusBadRequest, "start", err, botLogTarget(h.runtime.Config()), botLogMetadata(h.runtime.Config()))
		return
	}
	recordRequestOperation(c, h.logs, "start", "OneBot v11 机器人已启动", h.runtime.Config().ID, botLogMetadata(h.runtime.Config()))
	c.JSON(http.StatusOK, h.runtime.Status())
}
...
func (h *BotHandler) stop(c *gin.Context) {
	if err := h.runtime.Stop(); err != nil {
		h.writeError(c, http.StatusBadRequest, "stop", err, botLogTarget(h.runtime.Config()), botLogMetadata(h.runtime.Config()))
		return
	}
	recordRequestOperation(c, h.logs, "stop", "OneBot v11 机器人已停止", h.runtime.Config().ID, botLogMetadata(h.runtime.Config()))
	c.JSON(http.StatusOK, h.runtime.Status())
}
...
func (h *BotHandler) requestBackfill(c *gin.Context) {
	...
	if !ok {
		h.writeError(c, http.StatusNotImplemented, "backfill", fmt.Errorf("runtime does not support manual history backfill"), botLogTarget(h.runtime.Config()), botLogMetadata(h.runtime.Config()))
		return
	}
	var payload struct {
	...
	if err := runtime.RequestHistoryBackfill(window); err != nil {
		h.writeError(c, http.StatusConflict, "backfill", err, botLogTarget(h.runtime.Config()), botLogMetadata(h.runtime.Config()))
		return
	}
	recordRequestOperation(c, h.logs, "backfill", fmt.Sprintf("已触发手动回补，窗口 %s", window), h.runtime.Config().ID, botLogMetadata(h.runtime.Config()))

...
// 落到对应的字段里。
//
// 身份和外观是硬要求，运行时不会替人设补这两样：机器人配置上的「名称」字段从不
// 进提示词（只用于合并转发的显示名），外观更是全项目没有第二个来源。人设不写，
// 它就不知道自己叫什么、长什么样，出图时每次画出来的都是另一个人。落到卡片上，
// 这两样分别由 name 和 description 承担。
//
// 提示词里刻意不出现平台名，也不出现说话的场合：项目同时接 OneBot、Telegram、
// 飞书、企业微信和 QQ 官方机器人，而人设存在共享人设库（assistant 包的
// persona_library.go）里，一份卡随时会被应用到另一个平台的机器人上，同一个机器人
// 又同时在多人会话和一对一私聊里说话。写死「运行在 QQ 群里」的卡换个平台就是错的，
// 写死「活在群里」的卡被私聊用到时同样是错的——两种错的成因是同一个：把运行时
// 上下文当成了角色设定。人设只回答「它是谁、怎么说话」，「在哪儿说、对着谁说」
// 由运行时给：场景（promptGroupScope）、发言者模板、平台差异（能不能渲染
// Markdown、怎么分条、消息标记，见 platformOutputRulesForConfig）都在回复时另行
// 注入。同一条道理见 assistant.identityAliasPrefix 那段注释。正因如此，下面的
// 「一律不要写」里才把输出格式和场景整段划给运行时。
const personaGenerateSystemPrompt = `你在为一个聊天机器人写一张角色卡。只输出一个 JSON 对象，字段是 SillyTavern V2 角色卡 data 的那几项：

{"name": "…", "description": "…", "personality": "…", "scenario": "…", "first_mes": "…", "mes_example": "…", "system_prompt": "…"}

各字段写什么：
- name：别人叫这个角色什么。给了名字就用那个名字，不要另起一个。
- description：身份与来历，接着是说话方式。这是全卡最长的一个字段，说话方式又是其中的大头，要写到别人照着就能模仿出这个声音。身份部分交代它是谁、什么来历，并写清外观形象，写到能照着画出来：发色发型、瞳色、常穿的衣服、随身或身上显眼的标志物，挑最有辨识度的几样——这是它给自己出图、被问「你长什么样」时唯一的依据。说话方式写清楚：句子偏长还是偏短、一句说几件事、说话的节奏；口语化到什么程度，爱用哪些词、不用哪些词——举出具体的词，不要只说「自然」「亲切」；情绪不同的时候带出来的语气词分别是什么；别人抛梗时怎么接。再分别交代它遇到这几种情况各自怎么反应：有人分享近况、有人求助、被夸、被怼或被戳穿、遇到自己不懂的事。最后点名几种它绝不会用的腔调——客服腔、说教腔、结尾总结、拿反问句收尾之类，各配一个具体的反例短句，反例用「」引起来，不要用全角括号。
- personality：三到五条真正会影响它怎么反应的倾向，用连贯的话写出态度，不要堆形容词，也不要写成逗号分隔的标签。
- scenario：它眼下的处境和与人相处的分寸——它现在过着什么样的日子、和人打交道时把自己放在什么位置，以及对主人、对熟人、对陌生人分别是什么态度和亲疏称呼。只写它自己的处境和立场，不要交代对话发生在哪儿。
...
- 时间日期、发言者标注、什么时候该开口这些运行时会自动补上的上下文。
- 说话的场合：不要写它待在哪儿、用的是哪个软件、对着一个人说话还是对着很多人说话，也不要给它安排固定的观众。同一张卡会被用到各种场合，场合由运行时补上；写死一种，换个场合就是错的。要提到对话里的另一方时，就说「对方」「别人」「大家」。

                    ...
                    <div ref="modelSelectRef" class="model-select">
                      ...
                      <div v-if="modelMenuOpen" class="model-menu">
                        ...
                        >
                          ...
                          <span class="model-option-meta">{{ option.name || option.owned_by || "模型" }}</span>
                        </button>
                  ...
                  <div class="subpanel span-all">
                    ...
                    <div v-if="llmAdvancedOpen" class="subpanel-body">
                      ...
                      <label v-if="llmForm.provider === 'openai_compatible'" class="field span-all">
                        ...
                        <input v-model.trim="llmForm.userAgent" autocomplete="off" placeholder="codex-cli/0.142.0" />
                      </label>
                      ...
                      <label class="field">
                        ...
                        <input v-model.number="llmForm.maxOutputTokens" type="number" min="0" step="1" />
                      </label>
      ...
      >
        ...
        <aside class="bot-detail-panel" aria-label="机器人详情">
          <header class="bot-detail-head">
            <h3>机器人详情</h3>
            <button class="icon-button" type="button" aria-label="关闭详情" title="关闭详情" @click="resetBotChanges">
              <X :size="16" aria-hidden="true" />
            </button>
          </header>
          ...
          <div class="bot-detail-tabs" role="tablist" aria-label="机器人详情标签">
            <button
              v-for="tab in botDetailTabs"
              :key="tab.value"
              type="button"
              role="tab"
              :aria-selected="botDetailTab === tab.value"
              @click="botDetailTab = tab.value"
            >
              {{ tab.label }}
            </button>
          </div>
          ...
          <form v-if="botDetailTab === 'config'" class="bot-detail-body" @submit.prevent="onSaveBot">
            ...
            <label class="bot-detail-row">
              <span>消息格式</span>
              <select v-model.number="botForm.directReplyChunkSize">
                <option :value="500">Markdown</option>
                <option :value="900">纯文本</option>
                <option :value="1200">长文本</option>
              </select>
            </label>
            <label class="bot-detail-row">
              <span>代理 / 网络</span>
            ...
            <div class="bot-detail-toggles">
              <label>
                <span>
                  <strong>启用机器人</strong>
                  <small>允许接收并处理消息</small>
                </span>
                <input v-model="botForm.enabled" type="checkbox" />
              </label>
              <label>
                <span>
                ...
                <input v-model="botForm.agentEnabled" type="checkbox" />
              </label>
              <label>
                <span>
                  <strong>启用欢迎语</strong>
                ...
                <input v-model="botForm.welcomeEnabled" type="checkbox" />
              </label>
              <label>
                <span>
                  <strong>自动重连</strong>
                ...
                <input v-model="botForm.noneBotBridgeEnabled" type="checkbox" />
              </label>
            </div>
            ...
            <label class="bot-detail-textarea">
              <span>欢迎语</span>
              <textarea v-model="botForm.welcomeMessage" />
            </label>
          </form>
          ...
          <form v-else class="bot-detail-body bot-test-form" @submit.prevent="onSendGroupTest">
            ...
            <div class="bot-test-actions">
              <button class="button" type="button" :disabled="refreshingGroupTest || !botGroupTest.groupID" @click="onRefreshGroupTest">
                <RefreshCw :size="16" aria-hidden="true" />
                <span>{{ refreshingGroupTest ? "刷新中" : "刷新记录" }}</span>
              </button>
              <button class="button primary" type="submit" :disabled="sendingGroupTest || !botFeatures.group_test || !botGroupTest.groupID || !botGroupTest.message.trim()">
          ...
          <footer class="bot-detail-actions">
            <button class="button" type="button" @click="onTestBotConnection(editingBotProfile)">
              <Activity :size="16" aria-hidden="true" />
              <span>测试连接</span>
            </button>
            <button class="button" type="button" :disabled="!botForm.id" @click="onCloneBotProfile(editingBotProfile)">
              <Copy :size="16" aria-hidden="true" />
              <span>复制配置</span>
            </button>
            <button class="button primary" type="button" :disabled="savingBot || !botDirty" @click="onSaveBot">
              <Save :size="16" aria-hidden="true" />
...
const imageModelPresets: Record<Provider, string[]> = {
  openai_compatible: ["gpt-image-1", "gpt-image-1-mini", "gpt-image-1.5", "gpt-image-2", "dall-e-3", "dall-e-2"],
...
const textModelPresets: Record<Provider, LLMModelInfo[]> = {
  openai_compatible: [{ id: "gp5.5", name: "Default" }],
  gemini: [
...
type TabID = "llm" | "test" | "qqbot" | "group-admin" | "plugins" | "logs" | "theme";
...
type BotDetailTab = "config" | "events" | "messages" | "test";
...
function loadUIState() {
  ...
  try {
    ...
    if (state.botDetailTab === "config" || state.botDetailTab === "events" || state.botDetailTab === "messages" || state.botDetailTab === "test") {
...
function confirmDiscardChanges(kind: "LLM" | "机器人"): boolean {
  const dirty = kind === "LLM" ? llmEditorMode.value === "edit" && llmDirty.value : botDirty.value;
  if (!dirty) {
    return true;
  }
...
function setProvider(provider: Provider) {
  ...
  if (!llmForm.userAgent || llmForm.userAgent === "codex-cli/0.142.0") {
...
function defaultUserAgent(provider: Provider): string {
  return provider === "openai_compatible" ? "codex-cli/0.142.0" : "";
}
...
function loadBotFormProfile(config: QQBotConfig) {
  ...
  botForm.directReplyChunkSize = config.direct_reply_chunk_size || 500;
  ...
  botForm.maxBotConcurrency = config.max_bot_concurrency || 5;
  ...
  botForm.agentMaxSteps = config.agent_max_steps || 4;
...
function createBotProfile() {
  ...
  loadBotFormProfile({
    ...
    max_reply_chars: 3500,
    direct_reply_chunk_size: 500,
    forward_reply_threshold: 900,
    recent_context_limit: 20,
    max_bot_concurrency: 5,
...
async function load() {
  try {
    const [llmConfig, botConfig, statusPayload, pluginPayload, featurePayload] = await Promise.all([
      getConfig(true),
      getQQBotConfig(),
      getQQBotStatus(),
      listPlugins(),
...
function onBeforeUnload(event: BeforeUnloadEvent) {
  if (!(llmEditorMode.value === "edit" && llmDirty.value) && !botDirty.value) {
    return;
  }

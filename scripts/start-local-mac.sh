...
load_runtime_env() {
	...
	while IFS= read -r line || [[ -n "$line" ]]; do
		...
		case "$key" in
			DIANA_*|LLM_*|QQBOT_*|ONEBOT_*|NONEBOT_*|AGENT_*|EXA_API_KEY*|TAVILY_API_KEY*|APP_DB_PATH|LOG_PATH|HOST|PORT|FRONTEND_DIST|BILI_SESSDATA|DOUYIN_CK|XHS_CK|RESOLVER_PROXY|NAPCAT_QQ)
				export "$key=$value"
				;;
		esac
	done < "$1"

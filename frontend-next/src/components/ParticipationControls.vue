...
import { defaultParticipationCooldownSeconds, participationPreset, participationPresetName, participationLevelOptions, type ParticipationPreferences } from "../participation";
...
const description = computed(() => {
  if (inherited.value) return "使用所属机器人的回复欲望和冷却设置。";
...
function select(name: string) {
  if (name === "inherit" && props.inheritable) {
    ...
    emit("update:modelValue", participationPreset(name, cooldownSeconds.value));
  }
}
...
<template>
  <div class="participation-controls">
    ...
    <span v-if="!inherited" class="hint">档位只调整参与倾向，切换时不会修改冷却时间。</span>

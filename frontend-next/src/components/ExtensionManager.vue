    ...
    <Modal v-if="editing" :title="`${existing ? '编辑' : '添加'} ${kind === 'skill' ? 'Skill' : 'MCP'}`" wide @close="closeEditor">
      <div class="extension-form">
        ...
        <template v-else-if="editPreset && !editAdvanced">
          ...
          <p v-if="Object.keys(editPresetMasks).length && !readonly && !revealed" class="hint">令牌只显示掩码，<button type="button" class="link-button" :disabled="revealing" @click="revealSecrets">显示明文</button>。</p>
...
async function revealSecrets(){revealing.value=true;try{const clean=snapshot.value===state();const data=await manageExtension<{headers?:Record<string,string>;env?:Record<string,string>;preset_secrets?:Record<string,string>}>({operation:'reveal',kind:'mcp',name:form.value.name});
